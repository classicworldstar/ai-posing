...
function cmanOM_JS_init(){
	...
	for(var i = 0; i < cmanOM_Obj.length; i++){
		...
		if (cmanOM_VAR['device'] == 'mobi') {
			// スマホなど
			if     ( cmanOM_Obj[i].addEventListener ){
				cmanOM_Obj[i].addEventListener('touchstart', cmanOM_JS_mdown, false);
				cmanOM_Obj[i].addEventListener('touchend', cmanOM_JS_mup, false);
				cmanOM_Obj[i].addEventListener('touchmove', cmanOM_JS_mmove, false);
			}
			else if( cmanOM_Obj[i].attachEvent ){
				cmanOM_Obj[i].attachEvent('ontouchstart', cmanOM_JS_mdown );
				cmanOM_Obj[i].attachEvent('ontouchend', cmanOM_JS_mup );
				cmanOM_Obj[i].attachEvent('ontouchmove', cmanOM_JS_mmove );
			}
		}else{
			// パソコンなど
			if     ( cmanOM_Obj[i].addEventListener ){
				cmanOM_Obj[i].addEventListener('mousedown', cmanOM_JS_mdown, false);
				cmanOM_Obj[i].addEventListener('mouseup', cmanOM_JS_mup, false);
				cmanOM_Obj[i].addEventListener('mousemove', cmanOM_JS_mmove, false);
				cmanOM_Obj[i].addEventListener('mouseout', cmanOM_JS_mout, false);
			}
			else if( cmanOM_Obj[i].attachEvent ){
				cmanOM_Obj[i].attachEvent('onmousedown', cmanOM_JS_mdown );
				cmanOM_Obj[i].attachEvent('onmouseup', cmanOM_JS_mup );
				cmanOM_Obj[i].attachEvent('onmousemove', cmanOM_JS_mmove );
				cmanOM_Obj[i].attachEvent('onmouseout', cmanOM_JS_mout );
			}
		}
